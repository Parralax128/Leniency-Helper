...
namespace Celeste.Mod.LeniencyHelper.UI
{
    public class WebScrapper
    {
        ...
        public static ConcurrentDictionary<string, TweakInfo> TweaksInfo = new();

        public static void LoadInfo()
        {
            foreach(string tweak in Module.LeniencyHelperModule.TweakList)
            {
                Thread temp = new Thread(() => ExtractTweakInfo(TweakSlider.TweakToUrl(tweak), tweak));
                temp.Start();
            }
        }
        private static async void ExtractTweakInfo(string url, string tweak)
        {
            TweakInfo info = new();
